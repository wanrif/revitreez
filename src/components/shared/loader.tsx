...
function SpinnerLoader({ size }: { size: number }) {
  return (
    <div className='relative' style={{ width: size, height: size }}>
      ...
      {/* Inner ring */}
      <div
        className='absolute inset-1 animate-spin rounded-full border-2 border-transparent border-b-accent border-l-accent/30'
  ...
  return (
    <div className='flex items-center' style={{ gap }}>
      {[0, 1, 2].map((i) => (
        <div
          key={i}
          className='animate-bounce rounded-full bg-primary'
          style={{
            width: dotSize,
            height: dotSize,
...
function PulseLoader({ size }: { size: number }) {
  return (
    <div className='relative' style={{ width: size, height: size }}>
      {/* Ripple effects */}
      <div
        className='absolute inset-0 animate-ping rounded-full bg-primary/24'
        style={{ animationDuration: '1.5s' }}
      />
      <div
        className='absolute inset-2 animate-ping rounded-full bg-accent/28'
