# RevitReez — Modern React + Bun + Vite

This project is a modern, high-performance React application using the latest
ecosystem tools and conventions for scalable, maintainable development.

## Tech Stack

- **Runtime:** [Bun](https://bun.sh/) (use `bun`, not `npm`)
...
```bash
bun dev          # Start dev server (VITE_PORT or 5173)
bun run build    # TypeScript check + production build
bun run lint     # Oxlint
bun run format   # Oxfmt (auto-sorts imports + Tailwind classes)
```
...
const mutation = useMutation({
  mutationFn: (data: CreateUserDto) =>
    post<User, CreateUserDto>('/users', data),
})
```
...
- **Formatting:** Oxfmt (`.oxfmtrc.json`) with import and Tailwind class sorting

---

For more details, see:

- [src/services/API_GUIDE.md](src/services/API_GUIDE.md) — API usage, error
  handling, pagination, file upload/download
- [src/components/layouts/providers.tsx](src/components/layouts/providers.tsx) —
  App providers
- [src/lib/router.tsx](src/lib/router.tsx) — Router config
